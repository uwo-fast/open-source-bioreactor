...
- [ ] decide the drive motor
  - the current 36GP-3530 runs 1154 rpm at the output and is driven at ~39% PWM duty with no speed feedback, which is outside every published operating point
  - **the replacement already registered does not fix this.** The 36PG-3429-5.2 (RM-ESMO-16Q) is rated 950 rpm, which on the 94.5 mm impeller is 4.70 m/s at the tip — more than twice the 2.03 m/s at which stirring stops paying for Chlorella. It is the right motor family and the wrong ratio
  - the family is one motor at ~7280 rpm with the ratio varied, so the sibling SKUs are the same part at another speed. **RM-ESMO-16N**, 385 rpm no-load at roughly 19:1, is the candidate: scaling by the 16Q's published rated/no-load of 0.679 puts it near 261 rpm rated, and 257 rpm on this impeller is 1.27 m/s — Leupold's measured Chlorella optimum is 1.26. Confirm against 16N's own datasheet before ordering; 0.679 is measured on the 5.2 variant only
  - target band, for whatever is chosen: **255 rpm** for the 1.26 m/s optimum, **410 rpm** where the 2.03 m/s break-even falls
  - verify the bolt circle against the printed mount first; the GA36Y is Ø29.2 against the registered 36GP's Ø27.6, and the registered 36PG is Ø28
- [ ] design the sparger
