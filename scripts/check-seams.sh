...

{
    ...
    echo "# renderer keeps the shared face instead of merging it. The lids' come from the pinned"
    echo "# bayonet-lock library, which is fetched rather than vendored, so that fix is upstream;"
    echo "# the probes' come from the transition meeting the collet. TODO.md carries both."
    cat "$seen"
} > "$tmp/new"
