...
module sparge_arm(
  ...
  assert(
    _taper < feed_height / 2,
    str("sparge_arm: a ", _boss, " mm boss on a ", tube, " mm tube tapers over ", _taper, " mm, most of a ", feed_height, " mm socket")
  );
  ...
  if (show_fluid_path)
    ...
    difference() {
      ...
      if (!is_undef(screw_tap_radius))
        translate([0, 0, _bend + _taper + (feed_height - _taper) / 2])
          rotate([-90, 0, 0])
