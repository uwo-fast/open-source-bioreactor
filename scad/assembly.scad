...
// ---------------------------------
// Peristaltic Pump Mount Parameters
// ---------------------------------
    ...
    rotate([ 0, 0, 45 ])
    {
        ...
        if (!jar_x_sec)
            circular_pattern(a = occupy_angle, n = number_of_lights / 2, r = base_jar_cut_diameter / 2, offset = 180)
                strip_light(width = light_width, depth = light_depth, length = light_length,
                            radius = light_window_radius, center = true);
    }
}
...
if (render_peris)
{
    translate([ 0, jar_diameter / 2 - peri_mount_outer_diameter, jar_height + base_floor_height + top_base_height ])
