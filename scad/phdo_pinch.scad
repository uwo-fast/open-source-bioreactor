...
union() {
  difference() {
    ...
    translate([0, 0, -wall_thickness]) {
      ...
      cylinder(h=tail_length, d1=tail_major_diameter, d2=tail_minor_diameter, $fn=64);
      translate([0, 0, tail_length]) cylinder(h=tail_length, d=tail_minor_diameter, $fn=64);
    }

    // Cut out pinch window
    translate([0, 0, -probe_body_lenth * (1 - (1 - ratio) / 2)]) scale([10, 1, 1]) {
        cylinder(h=probe_body_lenth * ratio, d1=probe_body_diameter * ratio, d2=probe_body_diameter * (ratio - 0.2), $fn=64);
      }
  ...
  translate([0, 0, -probe_body_lenth * (1 - (1 - ratio) / 2)])
    intersection() {
      ...
      difference() {
        cylinder(h=probe_body_lenth * ratio, d1=probe_body_diameter + wall_thickness * 0, d2=probe_body_diameter + wall_thickness * 2, $fn=64);
        translate([0, 0, -wall_thickness])
          cylinder(h=probe_body_lenth, d1=probe_body_diameter - wall_thickness, d2=probe_body_diameter, $fn=64);
      }

      // Intersection (same as cut out pinch window) to create pinch tabs
      scale([10, 1, 1]) {
        translate([0, 0, pinch_gap])
          cylinder(h=probe_body_lenth * ratio - pinch_gap, d1=probe_body_diameter * ratio - pinch_gap, d2=probe_body_diameter * (ratio - 0.2) - pinch_gap, $fn=64);
      }
    }
}
