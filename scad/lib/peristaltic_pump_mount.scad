  ...
  difference() {
    union() {
      ...
      translate([0, 0, body_height]) rotate([0, 180, 0])
          mounts(
            pump_mount_height, pump_thread_diameter, pump_thread_diameter, pump_bore_diatance, mount_width,
            pump_mount_scale
          );
      // base mounts
      rotate([0, 0, 45]) {
        mounts(
          base_mount_height, base_thread_diameter, base_head_diameter, base_bore_distance, mount_width,
          base_mount_scale
        );
      }
      rotate([0, 0, -45]) {
